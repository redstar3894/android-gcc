...
remove_path (edge e)
{
  ...
  loop_bm = sbitmap_alloc (number_of_loops ());
  sbitmap_zero (loop_bm);
  deleted_loop = XNEWVEC (struct loop *, nrem);
  nreml = 0;
  for (i = 0; i < nrem; i++)
    if (rem_bbs[i]->loop_father->header == rem_bbs[i])
      {
        SET_BIT (loop_bm, rem_bbs[i]->loop_father->num);
        deleted_loop[nreml++] = rem_bbs[i]->loop_father;
      }

  /* Do not explictly cancel loops which are nested in other cancelled
     loops.  These will be handled recursively within
     cancel_loop_tree.  */
  for (i = 0; i < nreml; i++)
    if (loop_outer (deleted_loop[i])
        && TEST_BIT (loop_bm, loop_outer (deleted_loop[i])->num))
      deleted_loop[i] = NULL;
  free (loop_bm);
