  ...
  NEXT_PASS (pass_early_local_passes);
    {
      ...
      NEXT_PASS (pass_tree_sample_profile);
      NEXT_PASS (pass_cleanup_cfg);
      NEXT_PASS (pass_tree_profile_dump);
      ...
      if (dump_file && current_function_decl)
	{
	  ...
	  fprintf (dump_file, "\n;; Function %s (%s)[%d:%d]%s\n\n", dname, aname,
          ...
          if (profile_info_available_p ())
            {
              fprintf (dump_file,
                       ";; Function Profile Summary: runs = %lld, run_max"
                       " = %lld, sum_max = %lld\n\n",
                       (long long) profile_info->runs,
                       (long long) profile_info->run_max,
...
execute_one_pass (struct opt_pass *pass)
{
  ...
  /* Dump call graph to file if -cgraph dump option is specified.  */
  if ((pass->static_pass_number != -1) && (dump_flags & TDF_CGRAPH))
    {
      FILE *cgraph_dump_file = aux_dump_begin (TDA_CGRAPH_VCG,
					       pass->static_pass_number);
      if (cgraph_dump_file)
	{
	  dump_vcg_cgraph (cgraph_dump_file,
			   PARAM_VALUE (PARAM_CGRAPH_DUMP_COUNT_THRESHOLD),
			   true);
	  aux_dump_end (TDA_CGRAPH_VCG, pass->static_pass_number,
			cgraph_dump_file);
	}
    }
