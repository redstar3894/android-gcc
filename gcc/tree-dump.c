...
/* Information for a particular auxiliary dump.  */
struct aux_dump_info
{
  /* Suffix to use for auxiliary dump.  */
  const char *const suffix;
  /* Phase for which this auxiliary dump file was last opened.  Used
     to determine whether the file should opened write or append.  */
  enum tree_dump_index phase;
};

/* Table of auxiliary dump information for each AUX_DUMP_INDEX
   value.  */
static struct aux_dump_info aux_dump_info[] = {
  {".cgraph.vcg", TDI_none}
};
...
/* Begin an auxiliary dump file of type AUX_INDEX for PHASE.  */

FILE *
aux_dump_begin (enum aux_dump_index aux_index, enum tree_dump_index phase)
{
  char *main_name, *aux_name;
  FILE *stream;

  if (phase == TDI_none || !dump_enabled_p (phase))
    return NULL;

  main_name = get_dump_file_name (phase);
  aux_name = concat (main_name, aux_dump_info[aux_index].suffix, NULL);
  stream = fopen (aux_name,
		  aux_dump_info[aux_index].phase == phase ? "a" : "w");
  if (!stream)
    error ("could not open auxiliary dump file %qs: %s", aux_name,
	   strerror (errno));
  aux_dump_info[aux_index].phase = phase;
  free (main_name);
  free (aux_name);
...
/* Finish an auxiliary tree dump of type AUX_INDEX for PHASE. STREAM
   is the stream created by aux_dump_begin.  */

void
aux_dump_end (enum aux_dump_index aux_index ATTRIBUTE_UNUSED,
	      enum tree_dump_index phase ATTRIBUTE_UNUSED, FILE *stream)
{
  fclose (stream);
}
