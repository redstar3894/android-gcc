...
/* A subroutine which removes REG_EQUAL notes that are rendered invalid by
   the code motion done in dead_or_predicable.  */
static void
remove_reg_eq_notes (basic_block other_bb, bitmap merge_set)
{
  unsigned int i, j;
  bitmap_iterator bi;
  EXECUTE_IF_SET_IN_BITMAP (merge_set, 0, i, bi)
    {
      struct df_reg_info *reg_info = DF_REG_EQ_USE_GET (i);
      df_ref the_ref;
      unsigned int n_insns = 0;
      rtx *insns;
      if (! reg_info)
        continue;
      insns = XNEWVEC (rtx, reg_info->n_refs);
      for (the_ref = reg_info->reg_chain; the_ref;
      ...
      for (j = 0; j < n_insns; j++)
        {
          rtx insn = insns[j];
          rtx note = find_reg_equal_equiv_note (insn);
          /* It's possible for the chain returned by DF_REG_EQ_USE_GET
             to contain the same instruction multiple times. In such cases,
             after we remove the note from the instruction the first time,
             find_reg_equal_equiv_note will return NULL for all successive
             mentions of this instruction. So ensure NOTE is non-NULL before
             trying to remove it.  */
          if (note)
		    ...
		    basic_block other_bb, basic_block new_dest, int reversep)
{
  ...
  if (n_validated_changes == 0)
	...
	fail = 1;
      ...
      if (! fail)
        remove_reg_eq_notes (other_bb, merge_set);
