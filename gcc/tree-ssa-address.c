...
/* Splits address ADDR into PARTS. IV_CAND is the selected iv candidate
   used in ADDR, and IS_CAND_BASE is a flag indicating if IV_CAND should
   be uses as base address.
...
addr_to_parts (aff_tree *addr, struct mem_address *parts,
               tree iv_cand, bool is_cand_base, bool speed)
{
  tree part;
  unsigned i;
		...
		tree iv_cand, bool is_cand_base, bool speed)
{
  ...
  addr_to_parts (addr, &parts, iv_cand, is_cand_base, speed);
