...
operand_equal_p (const_tree arg0, const_tree arg1, unsigned int flags)
{
  ...
  /* If either arg0 or arg1 doesn't have a type, but not both, they are not
     equal. This can happen when the routine is called when parsing the
     operands of a type that depends on a template parameter. */
				 ...
				 bool rhs_only)
{
  ...
  if (rhs != orig_rhs || lhs != orig_lhs)
    return fold_build2 (truthop_code, TREE_TYPE (cmpop), lhs, rhs);
  return NULL_TREE;
}
...
static tree
fold_unary_1 (enum tree_code code, tree type, tree op0)
{
  tree tem;
  tree arg0;
...
{
  tree tem = fold_unary_1 (code, type, op0);
  if (tem)
...
static tree
fold_binary_1 (enum tree_code code, tree type, tree op0, tree op1)
{
  enum tree_code_class kind = TREE_CODE_CLASS (code);
  tree arg0, arg1, tem;
  tree t1 = NULL_TREE;
  ...
  switch (code)
	...
	{
	  tem = merge_truthop_with_opposite_arm (0, arg0, arg1, true);
	  if (tem)
	    return fold_build2 (code, type, tem, arg1);
	}
	...
	{
	  tem = merge_truthop_with_opposite_arm (0, arg1, arg0, false);
	  if (tem)
	    return fold_build2 (code, type, arg0, tem);
	}
...
{
  tree tem = fold_binary_1 (code, type, op0, op1);
  if (tem)
    set_expr_folded_flag (tem);
  return tem;
}
...
static tree
fold_ternary_1 (enum tree_code code, tree type, tree op0, tree op1, tree op2)
{
  tree tem;
  tree arg0 = NULL_TREE, arg1 = NULL_TREE;
  enum tree_code_class kind = TREE_CODE_CLASS (code);
...
{
  tree tem = fold_ternary_1 (code, type, op0, op1, op2);
  if (tem)
    set_expr_folded_flag (tem);
  return tem;
}
