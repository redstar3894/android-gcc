...
/* Return true if profile information is being used. */

bool
profile_info_available_p (void)
{
  return (profile_info && (flag_branch_probabilities || flag_sample_profile));
}
	...
	if (!EDGE_INFO (e)->ignore && !EDGE_INFO (e)->on_tree)
	  {
		...
		if (e->count > profile_info->sum_max)
		  {
                    /* The function profile counts may be updated after summary data
                       gets dumped in multiple threaded programs.  */
