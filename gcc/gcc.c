...
static const struct option_map option_map[] =
 {
   ...
   {"--canonical-prefixes", "-canonical-prefixes", 0},
   {"--CLASSPATH", "-fclasspath=", "aj"},
   {"--combine", "-combine", 0},
   {"--comments", "-C", 0},
...
display_help (void)
{
  ...
  fputs (_("\
  -canonical-prefixes   Canonicalize paths when building relative\n\
...
process_command (int argc, const char **argv)
{
  ...
  /* Handle any -canonical-prefixes flag early, to assign the function
  ...
  for (i = 1; i < argc; i++)
    {
      ...
      else if (! strcmp (argv[i], "-canonical-prefixes"))
  ...
  for (i = 1; i < argc; i++)
	...
	;
      else if (strcmp (argv[i], "-canonical-prefixes") == 0)
        ;
