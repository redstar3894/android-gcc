	       ...
	       basic_block entry_block_map, basic_block exit_block_map)
{
  ...
  FOR_EACH_BB_FN (bb, cfun_to_copy)
    {
      ...
      if (flag_sample_profile
						  ...
						  (caller_fndecl))))
        {
          ...
          if (new_bb->confidence != LOW_CONFIDENCE) {
            num_bb_annotated++;
            if (new_bb->count > func_max_count)
              func_max_count = new_bb->count;
          }
          if (dump_file)
            fprintf (dump_file, "=========================\n\n");
        }
  ...
  if (flag_sample_profile && num_bb_annotated >= 1)
    sp_set_sample_profile (func_max_count);
...
optimize_inline_calls (tree fn)
{
  ...
  /* Smooth the profile after the function inlining.  */
