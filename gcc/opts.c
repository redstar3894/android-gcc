...
decode_options (unsigned int argc, const char **argv)
{
  ...
  /* Thread-safety analysis will not work properly if the annotated functions
     get inlined because we lose the information on the original function
     declarations. While we tried to arrange for the analysis to happen as
     early as possible, it cannot be moved earlier than SSA construction with
     the current implementation. Unfortunately an extra early inlining pass
     could take place even before SSA construction (e.g. when profile
     generation/use is enabled). In order to not generate bogus warnings,
     we disable thread safety analysis when this extra early inlining is
     enabled.  */
  if (warn_thread_safety && cgraph_gate_ipa_early_inlining ())
    {
      inform (input_location,
              "Thread safety analysis is disabled due to early inlining"
              " (possibly enabled by other profile or coverage options)");
		      ...
		      unsigned int lang_mask)
{
  ...
  switch (code)
    ...
    case OPT_gmlt:
	 ...
	 level explicitly, we will set it to TERSE in decode_options().  */
