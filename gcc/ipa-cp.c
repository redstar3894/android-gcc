      ...
      if (ipcp_node_is_clone (node))
	{
	  ...
	  scale_complement = REG_BR_PROB_BASE - scale;
          ...
          /* Negative scale complement can result from insane profile data
             ...
             the profile counters for this function are split (this has the
             advantage of context sensitivity). This can result in sum of
             incoming edge counts from this module being larger than callee
             instance's entry count.
          */
