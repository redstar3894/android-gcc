      ...
      else
	{
	  ...
	  /* Warn if shadowing an argument at the top level of the body.  */
		  ...
		  && TREE_CODE (x) != PARM_DECL)
		{
	      ...
	      if ((warn_shadow_local || warn_shadow_compatible_local) && !err)
		{
	  ...
	  /* Maybe warn if shadowing something else.  */
		       ...
		       && TREE_CODE (oldlocal) == VAR_DECL)
		{
                  ...
                  /* If '-Wshadow-compatible-local' is specified without other
                     -Wshadow flags, we will warn only when the type of the
                     shadowing variable (i.e. x) can be converted to that of
                     the shadowed variable (oldlocal). The reason why we only
                     check if x's type can be converted to oldlocal's type
                     (but not the other way around) is because when users
                     accidentally shadow an old local variable, more than
                     often they would use the variable thinking (mistakenly)
                     it's still the old decl. It would be rare that users
                     would use the variable in the place that expects the
                     old decl but thinking it's a new decl.  */
                  if (can_convert (TREE_TYPE (oldlocal), TREE_TYPE (x)))
                    warning_code = OPT_Wshadow_compatible_local;
                  else
                    warning_code = OPT_Wshadow_local;
