...
/* Return the cgraph_node of DECL if decl has definition; otherwise return
   ...
   is resolved to after linking/symbol resolution. Use this interface when
   building a call graph edge so that the call destination is properly set.  */
...
promote_static_var_func (unsigned module_id, tree decl, bool is_extern)
{
  ...
  /* Map the new name to the original name, so that the sample profile can
     be correctly attributed.  */
  if (flag_sample_profile)
    sp_add_funcname_mapping (assembler_name, orig_name);
