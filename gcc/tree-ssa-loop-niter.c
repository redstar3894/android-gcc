...
/* Returns the copy of OPND if it is defined by
   a copy assignment; otherwise returns OPND itself.  */

static inline tree
get_opnd_copy (tree opnd)
{
  ...
  if (gimple_assign_copy_p (def))
    ...
    {
      if (gimple_phi_num_args (def) != 1)
        return opnd;

      return gimple_phi_arg_def (def, 0);
    }
  else
    return opnd;
}

/* Returns true if OPND1 and OPND2 are equal, or
   one is a copy of another.  */

static bool
operand_equal_or_copy_p (tree opnd1, tree opnd2)
{
  tree copy1, copy2;
  if (operand_equal_p (opnd1, opnd2, 0))
    return true;

  if (TREE_CODE (opnd1) != SSA_NAME
      || TREE_CODE (opnd2) != SSA_NAME)
    return false;

  copy1 = get_opnd_copy (opnd1);
  copy2 = get_opnd_copy (opnd2);

  /* No need to compare again.  */
  if (copy1 == opnd1 && copy2 == opnd2)
    return false;

  return operand_equal_p (copy1, copy2, 0);
}
			   ...
			   bounds *bnds)
{
  ...
  if (operand_equal_or_copy_p (varx, varc1))
    {
  ...
  if (!operand_equal_or_copy_p (varx, varc0)
      || !operand_equal_or_copy_p (vary, varc1))
    goto end;
