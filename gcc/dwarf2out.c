...
typedef struct die_struct GTY ((chain_circular ("%h.die_sib")))
{
  ...
  GTY ((desc ("use_dwarf4_extensions"))) die_id;
...
typedef struct comdat_type_struct GTY(())
{
  dw_die_ref root_die;
  dw_die_ref type_die;
...
typedef struct skeleton_chain_struct GTY(())
{
  dw_die_ref old_die;
  dw_die_ref new_die;
...
print_die (dw_die_ref die, FILE *outfile)
{
  ...
  if (use_dwarf4_extensions && die->die_id.die_type_node)
    {
      int i;
      char *sig = die->die_id.die_type_node->signature;
  ...
  for (ix = 0; VEC_iterate (dw_attr_node, die->die_attr, ix, a); ix++)
	...
	case dw_val_class_die_ref:
	  if (AT_ref (a) != NULL)
	    {
	      if (use_dwarf4_extensions
	          && AT_ref (a)->die_id.die_type_node)
	        {
	          int i;
	          char *sig = AT_ref (a)->die_id.die_type_node->signature;

		  fprintf (outfile, "die -> signature: ");
                  for (i = 0; i < DWARF_TYPE_SIGNATURE_SIZE; i++)
                    fprintf (outfile, "%02x", sig[i] & 0xff);
                }
	      else if (! use_dwarf4_extensions
...
build_abbrev_table (dw_die_ref die)
{
  ...
  for (ix = 0; VEC_iterate (dw_attr_node, die->die_attr, ix, a); ix++)
	...
	&& AT_ref (a)->die_mark == 0)
      {
	gcc_assert (use_dwarf4_extensions
	            || AT_ref (a)->die_id.die_symbol);
  ...
  for (ix = 0; VEC_iterate (dw_attr_node, die->die_attr, ix, a); ix++)
	...
	case dw_val_class_die_ref:
	  if (AT_ref_external (a))
	    size += (use_dwarf4_extensions
	             ? DWARF_TYPE_SIGNATURE_SIZE : DWARF2_ADDR_SIZE);
	  else
	    size += DWARF_OFFSET_SIZE;
	  break;
	case dw_val_class_fde_ref:
...
value_format (dw_attr_ref a)
{
  switch (a->dw_attr_val.val_class)
    ...
    case dw_val_class_die_ref:
      if (AT_ref_external (a))
	return use_dwarf4_extensions ? DW_FORM_sig8 : DW_FORM_ref_addr;
      else
	return DW_FORM_ref;
    case dw_val_class_fde_ref:
      return DW_FORM_data;
...
output_die (dw_die_ref die)
{
  ...
  /* If someone in another CU might refer to us, set up a symbol for
     them to point to.  */
  if (! use_dwarf4_extensions && die->die_id.die_symbol)
	      ...
	      if (use_dwarf4_extensions)
	        {
	          ...
	          for (i = 0; i < DWARF_TYPE_SIGNATURE_SIZE; i++)
	            dw2_asm_output_data (1, type_node->signature[i],
	                                 "%s", name);
	        }
	      else
	        {
	          ...
	          gcc_assert (sym);
	          dw2_asm_output_offset (DWARF2_ADDR_SIZE, sym,
...
output_comdat_type_unit (comdat_type_node *node)
{
  ...
  output_compilation_unit_header ();
  for (i = 0; i < DWARF_TYPE_SIGNATURE_SIZE; i++)
    dw2_asm_output_data (1, node->signature[i], "Type Signature");
  ...
  if (generate_debug_line_table && line != 0)
      ...
      if (DWARF2_ASM_LINE_DEBUG_INFO)
	{
	  ...
	  if (SUPPORTS_DISCRIMINATOR
	      && (use_dwarf4_extensions || !dwarf_strict)
	      && discriminator != 0)
...
{
  if (flag_eliminate_dwarf2_dups && ! use_dwarf4_extensions)
    {
