...
#define K_POS(b)        ((b))
#define K_NEG(b)        (50 * (b))
#define CORRECT_PENALTY 50
#define REVERSE_PENALTY 3
#define EDGE_COST       1
#define COST(k, w)      ((k) / mcf_ln ((w) + 2))
#define S_COST(m, w, i) ((m) * (((w) + 1000 * (i))))
...
typedef enum
{
  UNKNOWN,                      /* Cannot predict the count of the edge */
  CORRECT,                      /* The edge count is predicted to be correct */
  TOO_SMALL,                    /* The edge count is predicted to be too small */
  TOO_LARGE                     /* The edge count is predicted to be too large */
} predict_type;
...
static int
compute_bb_num_ir (basic_block bb)
{
...
create_fixup_graph (fixup_graph_type *fixup_graph)
{
  ...
  /* In create_fixup_graph: Each basic block and edge can be split into 3
     edges. Number of balance edges = n_basic_blocks - 1. And there're 1 edge
    ...
    if (flag_sample_profile)
      {
        if (bb->confidence == LOW_CONFIDENCE)
        ...
        else
          {
            ...
            fcost = (gcov_type) S_COST (mult, bb->count,
                                        compute_bb_num_ir (bb));
            predict = CORRECT;
          }
      }
    else
      ...
      if (!r_pfedge && pfedge->weight)
	{
	  /* Skip adding reverse edges for edges with w(e) = 0, as its maximum
	     capacity is 0.  */
          if (flag_sample_profile)
            {
              if (pfedge->predict == CORRECT)
                fcost = REVERSE_PENALTY * pfedge->cost;
              else
                fcost = pfedge->cost;
            }
          else
	  ...
	  add_fixup_edge (fixup_graph, pfedge->dest, pfedge->src,
			  REVERSE_EDGE, 0, fcost, pfedge->weight, UNKNOWN);
	}
    }
