...
static inline bool
maybe_hot_frequency_p (struct function *func, int freq)
{
  ...
  if (profile_status_for_function (func) == PROFILE_ABSENT)
    return true;
...
static inline bool
maybe_hot_count_p (struct function *func, gcov_type count)
{
  if (profile_status_for_function (func) != PROFILE_READ)
    return true;
...
maybe_hot_bb_p (const_basic_block bb)
{
  if (profile_status == PROFILE_READ)
    return maybe_hot_count_p (cfun, bb->count);
  return maybe_hot_frequency_p (cfun, bb->frequency);
}
...
bool
maybe_hot_bb_for_func_p (struct function *func, const_basic_block bb)
{
  if (profile_status_for_function (func) == PROFILE_READ)
    return maybe_hot_count_p (func, bb->count);
  return maybe_hot_frequency_p (func, bb->frequency);
}
